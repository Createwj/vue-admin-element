  ...
  <div class="app-container calendar-list-container">
    ...
    <el-form ref="listQuery" :inline="true" :model="listQuery" label-width="120px">
      <div class="filter-container">
        ...
        <el-form-item label="积分:">
          <el-input v-model="listQuery.integral" size="small" class="filter-item" placeholder="积分" />
        </el-form-item>
    ...
    <div class="table-box">
      ...
      <el-table :key="tableKey" v-loading.body="listLoading" :data="dataList" :header-cell-style="{background:'#F5F5F5'}" border fit highlight-current-row style="width: 100%">
        ...
        <el-table-column align="center" label="积分类型">
          <template slot-scope="scope">
            <span>{{ scope.row.integralSta }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="$t('table.'+textMap[dialogStatus])" :visible.sync="dialogAddOrUpdateVisible">
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="积分类型" prop="integralSta">
          <el-input v-model="form.integralSta" placeholder="请输入积分类型" />
        </el-form-item>
    ...
    <el-dialog :title="$t('table.'+textMap[dialogStatus])" :visible.sync="dialogInfoVisible">
      <el-form ref="form" :model="form" label-width="100px">
        ...
        <el-form-item label="积分类型" prop="integralSta">
          <el-input v-model="form.integralSta" readonly="readonly" />
        </el-form-item>
        <el-form-item label="录入时间" prop="inputTime">
          <el-input v-model="form.inputTime" readonly="readonly" />
        </el-form-item>
        <el-form-item label="备注" prop="remak">
          <el-input v-model="form.remak" readonly="readonly" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
...
<script>
import { page, addObj, getObj, delObj, putObj } from '@/api/admin/memberAccountLog'
...
export default {
  ...
  methods: {
    ...
    handleEmpty() {
      ...
      this.listQuery.integralSta = undefined
      this.listQuery.inputTime = undefined
      this.listQuery.remak = undefined
    },
    // 搜索
    handleFilter() {
      this.listQuery.page = 1
    ...
    handleUpdate(row) {
      getObj(row.memberId).then(res => {
        ...
        this.form.integralSta = data.integralSta
        this.form.inputTime = data.inputTime
        this.form.remak = data.remak
        this.dialogAddOrUpdateVisible = true
